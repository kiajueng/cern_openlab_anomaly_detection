...
pbeast_read_repository -r /eos/atlas/atlascerngroupdisk/tdaq-opmon/pbeast -p "$partition" -c "$class" -a "$attribute" -s "${year}-${month}-${days} 00:00:00" -t "${year}-${month}-${days} 00:01:00" -O "$regex" > "${BASE_DIR}/tmp.csv"
